 ...
 *
 * @uses $type string What kind of page should we use to construct urls (category, series, new)?
 * @uses $parent Category A parent category if one exists
 * @uses $currentTitle string The title to use for the current page.
 * @uses $currentTitleKey string Translation key for title of current page.
 *}

<nav class="cmp_breadcrumbs cmp_breadcrumbs_catalog text-xs sm:text-sm font-medium my-2" role="navigation" aria-label="{translate key="navigation.breadcrumbLabel"}">
	<ol class="not-prose inline-flex flex-wrap items-center gap-1 px-3.5 py-1.5 rounded-full bg-slate-100/90 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-800/80 text-slate-600 dark:text-slate-300 shadow-2xs backdrop-blur-md max-w-full">
		...
		{if $parent}
			<li class="inline-flex items-center space-x-1">
				<a class="text-slate-600 dark:text-slate-400 hover:text-{$activeTheme->getBaseColour()}-600 dark:hover:text-{$activeTheme->getBaseColour()}-400 transition-colors font-medium" href="{url op=$type path=$parent->getPath()}">
		...
		<li class="inline-flex items-center min-w-0 max-w-xs sm:max-w-md" aria-current="page">
			<span class="truncate px-2.5 py-0.5 rounded-md bg-{$activeTheme->getBaseColour()}-500/10 text-{$activeTheme->getBaseColour()}-700 dark:text-{$activeTheme->getBaseColour()}-300 font-bold">
				{if $currentTitleKey}
