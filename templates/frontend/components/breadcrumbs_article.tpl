 ...
 *
 * @brief Display a breadcrumb nav item showing the current article location.
 *
...
<nav class="cmp_breadcrumbs text-xs sm:text-sm font-medium my-2" role="navigation" aria-label="{translate key="navigation.breadcrumbLabel"}">
	<ol class="not-prose inline-flex flex-wrap items-center gap-1 px-3.5 py-1.5 rounded-full bg-slate-100/90 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-800/80 text-slate-600 dark:text-slate-300 shadow-2xs backdrop-blur-md max-w-full">
		...
		<li class="inline-flex items-center space-x-1">
			<a class="text-slate-600 dark:text-slate-400 hover:text-{$activeTheme->getBaseColour()}-600 dark:hover:text-{$activeTheme->getBaseColour()}-400 transition-colors font-medium" href="{url page="issue" op="archive"}">
				{translate key="navigation.archives"}
			</a>
			<svg class="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 shrink-0 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
		</li>
		{if $issue}
			<li class="inline-flex items-center space-x-1 max-w-xs truncate">
				<a class="text-slate-600 dark:text-slate-400 hover:text-{$activeTheme->getBaseColour()}-600 dark:hover:text-{$activeTheme->getBaseColour()}-400 transition-colors font-medium truncate" href="{url page="issue" op="view" path=$issue->getBestIssueId()}">
		...
		<li class="inline-flex items-center min-w-0 max-w-xs sm:max-w-md" aria-current="page">
			<span class="truncate px-2.5 py-0.5 rounded-md bg-{$activeTheme->getBaseColour()}-500/10 text-{$activeTheme->getBaseColour()}-700 dark:text-{$activeTheme->getBaseColour()}-300 font-bold" title="{if $currentTitleKey}{translate key=$currentTitleKey}{else}{$currentTitle|escape}{/if}">
				{if $currentTitleKey}
